...
#修改(加入影像辨識參數)
from model.Predict_price import predict_price
...
def detail(id):
    ...
    predicted_price = predict_price(listing["id"])
    ...
    # 限制 same_district_listings 為最多 6 條
    same_district_listings = [house for house in all_listings if house['district'] == district][:6]

    # 取得推薦列表並限制為最多 6 條
    
    recommended_listings = knn_similar_listings(all_listings, listing)[:6]
